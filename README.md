...
- On every boot, required workflow custom-node repos are ensured in `<COMFYUI_DIR>/custom_nodes` if missing (`/workspace/ComfyUI` with network volume, `/ComfyUI` without it).
