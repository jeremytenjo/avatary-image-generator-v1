...
capture_previous_project_state_for_switch() {
    ...
    PREVIOUS_PROJECT_MANIFEST_PATH=""
    ...
    PREVIOUS_PROJECT_MANIFEST_PATH="$SAVED_PROJECT_MANIFEST_PATH"
    PREVIOUS_PROJECT_SOURCE_URL="${SAVED_PROJECT_SOURCE_URL:-}"

    if [ -f "$PREVIOUS_PROJECT_MANIFEST_PATH" ]; then
        PREVIOUS_MANIFEST_SNAPSHOT_PATH="$(mktemp /tmp/dynamic-comfyui-previous-project-manifest.XXXXXX.yaml)"
        cp -f "$PREVIOUS_PROJECT_MANIFEST_PATH" "$PREVIOUS_MANIFEST_SNAPSHOT_PATH"
    fi

    export PREVIOUS_PROJECT_KEY PREVIOUS_PROJECT_MANIFEST_PATH PREVIOUS_PROJECT_SOURCE_URL PREVIOUS_MANIFEST_SNAPSHOT_PATH
    return 0
}
